...
- **架构**：纯静态单文件 `web-admin/index.html`（无构建步骤）。⚠️ **不加载 `@cloudbase/js-sdk`**——2026-09-13 单日内连续四轮真机排查（证书报错→匿名登录被拒绝→`callFunction` 内部 `null.scope` 崩溃两版补丁均无效）后，用 Node 下载 CDN 上真实的 `cloudbase.full.js@2.28.6` 搭桩件复现，字节级定位到 SDK 内部凭证状态机的一个判空顺序颠倒 bug，打补丁消除崩溃后进一步确认：这版 SDK 的 `callFunction()` 无论如何都要求客户端本地先有一次成功的登录握手（哪怕匿名登录）才能拿到凭证，与本项目环境"只开启未登录访问权限、不开启匿名登录身份源"这个组合从架构上不兼容，任何客户端补丁都无法绕过。最终改为用浏览器原生 `fetch()` 直连 SDK 内部实际使用的云函数 HTTP 网关（`https://<envId>.<region>.tcb-api.tencentcloudapi.com/web`，`action:'functions.invokeFunction'`，协议细节从 SDK 源码逐层反推确认，不带 `access_token`），完整排查链路与网关协议细节见 `web-admin/README.md`。真正的操作权限完全由 `cloudfunctions/adminWebAuth` 登录后签发的会话令牌决定。
...
- **部署前置条件**（无法由代码验证，需在腾讯云开发控制台手动配置，详见 `web-admin/README.md`）：目标环境启用"未登录用户访问云资源权限"、**⚠️ 另外单独为 `adminWebAuth`/`adminWebConsole` 这两个云函数配置安全规则 `{"invoke": true}`**（真机实测确认：环境级开关只是总闸，云函数还有一层独立的、按函数配置的安全规则，默认要求登录，不满足会在网关层直接拒绝 `[PERMISSION_DENIED]`，且这一步**无法通过任何云函数的 `config.json` 声明**，只能在控制台"云函数 → 安全规则"里配置，只加这两条、不要动 `"*"` 通配符默认规则，否则会让全仓库所有云函数都能被匿名调用）、部署域名加入安全域名白名单、两个新云函数已部署且 `ADMIN_CONSOLE_INTERNAL_TOKEN` 与 `activateTenantSubscription` 侧配置的值完全一致。真机实测已确认绕开 `@cloudbase/js-sdk` 直连网关这个方案的请求 URL/Body 协议格式完全正确（请求真实打到了网关并收到结构化的 `PERMISSION_DENIED` 响应，不是网络层失败），当前唯一剩下的未知是配置好安全规则之后完整的登录+三个页面操作链路能否端到端跑通，需要你在控制台完成上述配置后实际测试确认。