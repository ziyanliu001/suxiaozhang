...
- **架构**：纯静态单文件 `web-admin/index.html`（无构建步骤），加载 `@cloudbase/js-sdk`（CDN，固定 `2.28.6` 版本），`cloudbase.init()` 之后**不调用** `auth.signInAnonymously()`——依赖目标环境已开启的"未登录用户访问云资源权限"（微信云开发控制台开关，与腾讯云独立版 CloudBase 的"匿名登录"身份源是两套不同机制，2026-09-13 曾误用后者、被环境直接拒绝"请联系开发者在身份源列表开启匿名登录"）。该版本 SDK 在完全没有登录凭证时，`callFunction()` 内部会反复调用 `auth.getLoginState()`，未登录时该方法本身返回 `null`，SDK 未做判空直接读 `.scope`，会在浏览器端崩溃（真机含无痕模式复测稳定复现，"提前调一次 `getSession()` 预热"这个思路已被证实无效）——`ensureApp()` 里给 `auth.getLoginState()` 打了一个轻量兜底补丁，解析出 `null`/抛异常时统一兜底成 `{ scope: '' }`，只中和这个 SDK 判空缺失的 bug，不产生、也不冒充任何登录态。真正的操作权限完全由 `cloudfunctions/adminWebAuth` 登录后签发的会话令牌决定。
