# 十方明账项目开发核心规范与 Agent 宪法 (CLAUDE.md)

> **🏢（2026-09-16 品牌与主体升级）**：小程序正式商用品牌名已由"素小账"变更为**"十方明账"**；运营主体由个人开发者迁移至**个体工商户"厦门市思明区曾图扣软件开发经营部"**（注意：是个体工商户，不是有限公司/企业法人）；小程序 AppID 已由 `wx967852422ebfce48` 迁移至新主体名下的 `wxb1bc12a24926ec2f`（`project.config.json` 已同步）。⚠️ **如实标注两处未随文本改名同步变更、需要人工核实的边界**：① 本地 Obsidian 知识库磁盘路径 `/home/ziyan/文档/Obsidian Vault/01-Projects/素小账/`（含入口 `00-素小账-MOC.md`）**仍沿用历史文件夹/文件名**，本文档下方与本节其余出现的这个路径字符串均未改名（改名需要用户手动操作 vault 磁盘文件，不在本次自动化范围内），阅读时请按"这是历史路径，指向的就是十方明账项目"理解，不要误以为路径打错；② 新 AppID 对应的微信支付商户号/云开发环境/mp 后台隐私协议文档等**外部平台配置**均需要用户在微信公众平台手动完成实名认证与关联，本次改动仅覆盖仓库内的代码与文案，不代表新主体已经完成微信侧的实际入驻流程；③ 原 AppID 的本地上传私钥 `private.wx967852422ebfce48.key` 对新 AppID **无效**，新 AppID 的上传私钥需要用户从微信公众平台新主体账号下重新生成下载，不能靠重命名旧文件冒充。
...
- **架构**：纯静态单文件 `web-admin/index.html`（无构建步骤）。⚠️ **不加载 `@cloudbase/js-sdk`**——2026-09-13 单日内经历八轮真机排查（完整链路见 `web-admin/README.md`「已知边界」，摘要：证书报错→匿名登录被拒绝→`callFunction` 内部 `null.scope` 崩溃两版补丁均无效→字节级定位到 SDK 凭证状态机的判空顺序颠倒 bug、确认这版 SDK 无论如何都要求先有一次成功的登录握手才能拿到凭证→反推 SDK 私有网关协议直连、真机确认协议格式正确但被云函数"安全规则"拒绝）后，最终改用腾讯云开发**官方文档化**的"HTTP 网关"能力：控制台为 `adminWebAuth`/`adminWebConsole` 各配置一条免鉴权 + CORS 的网关路由，页面用浏览器原生 `fetch()` 直接 POST 到 `https://<网关域名>/<云函数名>`，请求体就是业务数据本身（不需要额外包装字段），完全不依赖任何 SDK 或私有协议细节。真正的操作权限完全由 `cloudfunctions/adminWebAuth` 登录后签发的会话令牌决定。
- **HTTP 网关触发的参数形状差异**：通过网关调用时，云函数收到的请求体被封装在 `event.body`（字符串）里，与云端测试/`cloud.callFunction()` 直接调用时业务字段平铺在 `event` 上的形状不同——`adminWebAuth`/`adminWebConsole` 都新增了 `lib/normalizeGatewayEvent.js`（各自独立维护一份镜像，同样受"云函数间无共享模块机制"约束），在 `exports.main` 入口统一识别并解包两种触发方式。
...
- **本页面不直接读写数据库**：所有数据访问都通过 `fetch()` 直连云函数 HTTP 网关中转，云函数内部做真正的权限校验，浏览器端不持有任何数据库直接访问凭证，与本仓库其余云函数"服务端强校验、客户端不可信"的一贯架构保持一致，不因为是独立于微信生态的页面就降低这条标准。
- **部署前置条件**（无法由代码验证，需在腾讯云开发控制台手动配置，详见 `web-admin/README.md`）：目标环境启用"未登录用户访问云资源权限"；为 `adminWebAuth`/`adminWebConsole` 各配置一条 HTTP 网关路由（`/adminWebAuth`、`/adminWebConsole`，均需开启 CORS + 免鉴权）并把网关域名填入页面"连接设置"；部署域名加入安全域名白名单；两个新云函数已部署且 `ADMIN_CONSOLE_INTERNAL_TOKEN` 与 `activateTenantSubscription` 侧配置的值完全一致。**未做真机/真实浏览器联调测试，如实标注**——这是排查链路里第八轮方案，前七轮均被真机测试推翻或部分推翻过，这次改用官方标准能力理论上不确定性更低，但仍需你在控制台完成上述配置后实际测试一遍完整登录+三个页面的操作链路才能确认。

---

## 10. 微信小程序工业级 Vibe Coding 开发方法论与工程守则（2026-09-15 沉淀）

本节把本项目实际跑通的协作模式第一次成文，来源是第 1~9 节里已经真实发生过的具体案例（雨花斋四态审批流、义工修心积善打卡、四道应急防线等），不是抽象引入的外部方法论——本节只是给这套已经在跑的做法起名字、画拓扑，方便新接手的人和新 Agent 快速对齐，具体执行细节仍以第 0~9 节各自的条文为准，本节不重复也不覆盖它们。

### 10.1 微信专属 WAT 协同架构

- **W (Workflows)**：Obsidian 知识库（真实磁盘路径 `/home/ziyan/文档/Obsidian Vault/01-Projects/素小账/`）是业务规则唯一真源，`docs/` 只是手动同步的快照副本（详见第 1 节文档索引）。任何非平凡改动先在 Markdown 里把数据契约（对照 [`SCHEMA.md`](docs/SCHEMA.md)）与状态机（如阳光账本四态流转）推演清楚，再落代码——绝不拿着一句模糊需求直接从零盲敲。
- **A (Agents)**：Claude Code CLI 是核心决策与执行中枢，遵循"Engineer's Way"——每次接手任务前明确 **Role**（这个改动属于轨道一还是轨道二，见第 2 节）、**Context**（先读 `./docs/` 与真实代码现状，见 10.5 铁律六）、**负面清单**（本 CLAUDE.md 里已经写死的红线，如"克制原则""善款支付红线""自审自批熔断名单"），三者缺一都会导致方案跑偏。
- **T (Tools)**：微信云开发（CloudBase）是唯一后端基础设施；`node --test`（第 4 节 `npm test`）跑纯函数单测；`scripts/agent-check.js`/`scripts/security-audit.js` 是本地自闭环校验与安全审计闸门；Git 是物理钩子层，`.gitignore`（第 5 节）与 pre-push 类校验负责在提交前物理拦截敏感信息，不依赖人工记得检查。

### 10.2 五步协同流水线（The 5-Step WeChat Vibe Workflow）

1. **契约先行**：先定数据模型（对照 [`SCHEMA.md`](docs/SCHEMA.md) 字段清单）与状态机流转（如 `PENDING → APPROVED → AUDITED_LOCKED` 及 `isVoid` 并行分支，见第 2 节），不急于写代码。字段/状态一旦定型再动工，返工成本远低于代码写完再改数据模型。
2. **大脑约束**：把合规红线与负面清单写进本 CLAUDE.md（不是写进某个 Agent 的一次性 prompt 里，会话结束就丢），锁死安全边界——例如"善款不接支付网关""iOS 隐藏内购入口""自审自批熔断名单"这类规则一旦成文，后续任何一次接手任务的 Agent 都会自动读到，不需要每次由人复述一遍。
3. **前后一体**：Claude Code 本地接管，云函数（Node.js）+ 前端（WXML/WXSS/TS）+ 配套纯函数单测（`lib/*.js` + `lib/*.test.js`）一次性一体化生成，不把"补测试"当成事后可选的独立任务——测试与实现在同一次改动里交付，见 7.4 节 `sanitizeMeritTags` 的先例。
4. **机器把关**：`npm run agent:check` 三段式（typecheck → test → 通知开发者工具重新编译）任一环节非零退出即打回重做，不允许"先跑通看起来对就提交"；`.gitignore` 与 `scripts/security-audit.js` 是 pre-push 前的物理阻断层，专门防止私钥/口令/云开发密钥类敏感信息随手一提交就进了 git 历史（第 5 节安全红线）。
5. **双轨交付**：面向微信生态的改动走开发者工具本地预览 → 提审发布的标准通道；面向运营/应急场景的能力（如四道防线里的第四道）额外走一条独立于微信审核周期的静态托管 Web 中台（`web-admin/`，第 9.6 节），两条通道互不阻塞——微信审核变慢/被拒不影响应急接管能力随时可用。

### 10.3 六大避坑铁律（微信生态特定约束）

1. **双线程沙箱与 DOM 禁忌**：小程序逻辑层没有浏览器 DOM/BOM，`window`/`document`/`localStorage` 一律不存在，只能用 `wx.*` 原生 API（如 `wx.setStorageSync`）与 `this.setData` 驱动视图；`web-admin/`（第 9.6 节）是例外——它是独立于小程序运行时的纯浏览器页面，DOM API 在那个上下文里合法，不要把这条铁律错误套用到 `web-admin/`。
2. **资质与支付合规红线**：iOS 端必须物理隐藏虚拟支付/内购入口（`isIOSDevice()`，见第 3 节 SaaS 订阅小节）；`report_logs` 公益记账流水绝不接入任何在线支付网关，捐赠/随喜线下录入、零抽成（第 2 节"善款支付红线"）；对外生成的物料（收据、海报、系统文案）统一使用"乐捐"等民政合规表述，不使用带有宗教修行色彩的词汇——具体允许/禁止词清单见第 7.2 节去宗教化合规基线，新增文案前先核对该清单，拿不准时按更保守的方向选词。
3. **微信物理硬顶协议**：`wxacode.getUnlimited` 的 `scene` 参数官方硬限 32 字符，拼业务 ID 前必须先做 Base36 压缩（`getStoreQRCode` 的 `hexToBase36()`）并显式校验拼接后长度，同时传 `check_path: false`（见第 3 节）；涉及真实网络往返的云函数（图像/OCR/批量导出）`config.json` 必须显式配置 `timeout: 20` 秒以上，前端 `callFunctionWithTimeout` 同步调到 25000ms 左右——不写这个字段会静默走平台默认 3 秒，历史上大量 `-504003` 报错的根因都是这个默认值，不是业务逻辑本身慢。
4. **分包隔离与 DRY 平衡**：微信小程序分包之间不能相互引用，跨分包公共逻辑必须留驻主包 `utils/`；反过来，分包内部为了消除"和主包某个函数长得很像"这类静态 lint 告警而手动复制粘贴一份仅供本分包用的私有实现，是可以接受的权衡，不必为了 DRY 而违反分包边界去做不合法的跨分包 `require`。
5. **纯函数解耦驱动单测**：复杂业务判断/算法/数据转换从云函数 `index.js`（依赖 `wx-server-sdk` 运行时）里拆到独立的 `lib/*.js` 纯函数，配 `lib/*.test.js`，用 `node --test` 直接跑，不需要云端环境或 mock 整个 SDK——本仓库 `sanitizeMeritTags`/`validateClaim`/`grantAuthorizationRules` 等都是这个写法的先例，新增复杂逻辑优先照此拆分。
6. **读在写前（Read Before Write）**：接手任何任务前先用 Grep/Explore 类工具核实真实代码现状与 `./docs/` 文档，不要凭任务描述里的措辞或过去的印象直接假设某个字段/函数/机制存在——本仓库文档本身也会 drift（见 `feedback_claude_md_drifts_from_code` 记忆与第 1 节多处"⚠️ 已废止/已核实修正"标注），代码里也常有"看起来该造但其实已经存在"的既有实现（如 [[project_material_parser_already_existed]] 类先例），先读后写能同时避免重复发明轮子和延续已经过时的错误认知。

### 10.4 商业安全与四道防线拓扑

详细条文见第 9 节，本节只汇总拓扑关系供快速定位：
- **第一道**（`setupSuperAdmin`）：要求调用者已是 `platform_admin`，或系统内一个 `platform_admin` 都没有时自举——日常权限授予的默认通道。
- **第二道**（`emergencyClaimSuperAdmin`）：微信小程序内凭 `EMERGENCY_RECOVERY_SECRET` 密钥自助接管，fail-closed + 常量时间比较 + 失败锁定 + 高危审计日志，唯一超管账号失联时的应急手段（第 9.1~9.4 节）。
- **第三道**（`scripts/ops/grant-super-admin.js`）：完全脱离微信/云函数运行时，本地/CI 用 `@cloudbase/node-sdk` + 腾讯云 API 密钥直连数据库，信任模型比密钥字符串更强、因此业务校验更松（第 9.5 节）。
- **第四道**（`web-admin/` + `adminWebAuth`/`adminWebConsole`）：独立于微信生态的 Web 管理中台，账密登录 + HTTP 网关直连云函数，前三道防线都不可用（如小程序生态整体不可用）时的最后手段（第 9.6 节）。

四道防线共用同一套 `user_roles`/`audit_logs` 字段口径（各自独立 `require` 或镜像同一份纯函数校验逻辑），但触发条件、身份验证方式与信任模型各不相同——新增第五道防线前，先确认前四道是否已经覆盖了要解决的失效场景，不要重复造轮子。

### 10.5 与 Obsidian 知识库的关系

本节内容已同步收录进 Obsidian 知识库 `02-技术与契约/微信小程序工业级Vibe_Coding方法论.md`，并在 `00-素小账-MOC.md` 补了双链引用。`docs/` 快照与 vault 原文不会自动同步（见第 1 节），后续对本方法论的修订如果发生在某一侧，记得按第 0 节"落地与回写"手动同步另一侧。