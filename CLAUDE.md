...
- **架构**：纯静态单文件 `web-admin/index.html`（无构建步骤），加载 `@cloudbase/js-sdk`（CDN，固定 `2.28.6` 版本），`cloudbase.init()` 之后**直接** `app.callFunction()`，不调用 `auth.signInAnonymously()`——依赖目标环境已开启的"未登录用户访问云资源权限"（微信云开发控制台开关，与腾讯云独立版 CloudBase 的"匿名登录"身份源是两套不同机制，2026-09-13 曾误用后者、被环境直接拒绝"请联系开发者在身份源列表开启匿名登录"，修复后彻底移除了这条依赖）。真正的操作权限完全由 `cloudfunctions/adminWebAuth` 登录后签发的会话令牌决定。
