 ...
 * 因为它同时被 admin/personal/reports 三个分包引用——微信规则下"分包之间
 * 不能互相引用私有文件，只有主包资源可被所有分包共享"，主包一度是唯一
 * 合法的存放位置。但这会触发开发者工具"主包内不应存在主包未使用的JS
 * 文件"质量扫描告警（主包自身从未 import 过这个文件）。为彻底消除该告警、
 * 同时把这部分体积从主包移出，改为在每个消费方分包内各放一份独立拷贝
 * （与 recentPages.ts 在 admin/reports 两个分包各自维护一份是同一种既有
 * 惯例）。
 *
 * ⚠️ 与 subpackages/personal/utils/navGuard.ts、
 * subpackages/reports/utils/navGuard.ts 是三处独立维护的镜像，行为必须
 * 保持一致——改动一处记得同步其余两处。
 *
 * 业务场景：
