...
// 传了就一定生效，也不需要在这里重复校验。
import { createOrderAndPay } from '../../utils/wxPayCore';
...
Page({
  data: {
    ...
    product: null as { name: string; priceYuan: string; dailyCapacityLimit: number; leadTimeDays: number } | null,
  ...
  async onTapOrder() {
    ...
    try {
      const outcome = await createOrderAndPay(
