 ...
 *
 * ⚠️（2026-09-13 排查结论，勿再"清理"）微信开发者工具代码质量面板会报
 * "主包内不应存在主包未使用的JS文件"——本文件确实只被 admin/personal/
 * reports 三个分包的页面引用，主包自身从未 import 过。这不是死代码，而是
 * 微信"分包之间不能互相引用私有文件、只有主包资源可被所有分包共享"这条
 * 硬规则下唯一合法的存放位置：挪进任意一个分包都会导致其余分包的 import
 * 编译失败；复制成三份分别塞进各分包又会把同一份逻辑拆成三处手动同步。
 * 已与项目 owner 确认：接受这条面板告警作为已知的、无害的体积优化提示，
 * 不做代码改动。
 *
