...
//
// ⚠️（2026-09-13 排查结论，勿再"清理"）与同目录 navGuard.ts 头部注释同一处
// 结论：本文件只被 factory/personal 两个分包引用、主包自身从未 import，会
// 触发开发者工具"主包内不应存在主包未使用的JS文件"面板告警，但这是微信
// "分包间不能互相引用私有文件、只有主包资源可被所有分包共享"规则下唯一
// 合法的存放位置，已与项目 owner 确认接受该告警，不做代码改动。
//
