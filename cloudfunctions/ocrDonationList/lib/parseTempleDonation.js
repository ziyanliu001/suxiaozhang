...
// 🪞 镜像文件：与 miniprogram/utils/lib/parseTempleDonation.js 是同一份纯逻辑
// 的独立拷贝——"云函数间无共享模块机制"（云函数各自独立部署打包），本仓库
// resolveCaller.js/buildAuditLogEntry.js 等已有多处同类镜像，这是又一个实例，
// 改动时两处需要手动同步。
//
// 纯逻辑：宫庙红榜"姓名+传统用语(+具体事项)+金额/数量单位"识别，供
// ocrDonationList/index.js 识别手写宫庙红榜截图（区别于该云函数原有的微信
// 群收款/接龙截图识别场景）。
