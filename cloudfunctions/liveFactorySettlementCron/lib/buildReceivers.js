...
//
// 🏛️（护城河三 M2）本文件与 completeProductionOrder/lib/buildReceivers.js
// 内容完全一致——本仓库云函数间无共享模块（各自独立部署），T+1 兜底扫描需要
// 复用同一套"翻译成接收方数组"的逻辑，只能原样复制一份。如果未来
// completeProductionOrder 那份的分账口径变了（比如新增第三方接收人），这份
// 也要同步改，否则 T+1 兜底会用旧口径重新分账。
//
// 🛡️ 只分账"有明确接收人"的部分，不猜：
