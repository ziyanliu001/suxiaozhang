...
// 🐛 补齐"应收"：部分 POS 小票用"应收金额"表达最终应付款（与"应付"同义，
// 商家视角"应当收到"= 顾客视角"应当付出"），此前只认"应付"，遇到只印
// "应收"字样的小票会连 TIER2 都命中不了，只能退回明细累加兜底，更容易
// 因为累加误差与实收金额对不上
const TIER2_KEYWORD_REGEX = /(?:实收|应收|应付|合计|小计)\s*[:：]?\s*([¥￥]?\s*\d+(?:\.\d{1,2})?)/i;
...
const TIER2_BARE_KEYWORD_REGEX = /^(?:实收|应收|应付|合计|小计)\s*[:：]?\s*$/i;
...
// 🐛（2026-08-31 追加修复"9.75 被识别成 19.51"）补齐"原价/现价/折扣"：与
// "优惠"同理——部分小票会打印"原价:9.76 现价:9.75"这类逐件比价行，"原价"
// 本身不是最终关键词正则的目标词，但它的数值紧跟在冒号后面，一旦这一行
// 恰好没有被"总金额"那条硬排除逻辑挡住，且后面 TIER2 的"合计"子串意外命中
// 了"原价合计"这种复合词（"合计"是它的子串），就会把折前原价错认成最终
// 支付额。与"优惠"用同一套豁免逻辑：只有当整行确实不含 TIER1/TIER2 真正
// 的关键词时才排除，避免误伤"原价已扣除优惠后实付57.30"这类本身就是合法
// 实付行的句子。
function isExcludedLine(line) {
  if (/总金额/.test(line)) return true;
  if (/优惠|原价|现价|折扣/.test(line) && !TIER1_KEYWORD_REGEX.test(line) && !TIER2_KEYWORD_REGEX.test(line)) return true;
  return false;
}
...
// 🐛 补齐"应收/找零/服务热线"：均为小票底部结算区专属词汇（不会出现在任何
// 食材品名里），与既有的"应付/合计"同一性质，一并纳入截断标记，防止这几行
// 被当成商品明细继续往下解析
const SECTION_END_MARKERS = /商品数量合计|---其他---|商品小计|商品实付|在线支付|实付|支付宝|应付|应收|找零|服务热线|合计|小计|总计|总金额|配送运费|配送费|运费|优惠券|优惠|满减|立减/;
...
exports.main = async (event, context) => {
  ...
  try {
    ...
    // 🐛（2026-08-31 追加修复"9.75 被识别成 19.51"）根因：部分小票会用
    // "原价:12.50""现价:9.75"这类逐件比价行标注折扣（原价划线价 + 折后价
    // 各占一行），此前 isNoiseLine 完全不认识裸词"原价/现价"（只有复合词
    // "原价合计"在列表里）——这两行含中文字符，会被下面 isNameLine 误判成
    // 两个全新的"商品"（品名分别是"原价""现价"，价格分别是 12.50/9.75），
    // 而真正的商品名（如"苹果"）因为紧跟着的"原价"行触发了 flush 却没采集
    // 到任何价格数字（currentBlockNumbers 为空、非白名单）而被静默丢弃。
    // 一旦小票本身没有独立的"实付/合计"汇总行（部分单件小票只打印比价，没有
    // 结算区），金额提取会退回明细累加 sumExtracted，"原价12.50+现价9.75"
    // 两个假条目相加 ≈ 实付金额的两倍附近，这正是"翻倍"报错的真实来源。
    // 补齐"原价/现价/优惠/折扣/应收/找零/支付/光临(覆盖'欢迎光临'/'欢迎下次
    // 光临'等所有变体)/服务热线"，让这些结算相关词汇统一被当噪声行处理——
    // 命中就 flush 收尾上一件商品并整行跳过，不再当作新商品名开出一条假明细。
    const isNoiseLine = (str) => {
      return /店号|工号|单号|品名|数量|单价|金额|售出商品|原价合计|原价|现价|为您节省|优惠|折扣|实收|应收|回找|找零|支付|微支付|微信支付|检索号|会员|销售时间|欢迎光临|光临|服务热线|请保留|存根|小票|合计|小计|总计|商品小计|商品实付|在线支付|实付|支付宝|应付/i.test(str);
    };
