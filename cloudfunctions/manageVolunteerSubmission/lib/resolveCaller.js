...
//
// ⚠️（2026-09-12）本文件是 manageStoreProfile/lib/resolveCaller.js 的第五处
// 镜像（与 getPatriarchDashboard、manageReportApproval、manageDailyMenu 并列）。
//
// 🐛 根因（首页"登记今日菜单与人数"报"无权限提交"）：manageVolunteerSubmission
// 原先只有一个不支持漫游的极简 resolveCaller(OPENID)——账号通过【选择服务
// 站点与身份】弹窗以 authorizedTenants 轻量租户漫游授权临时获得目标门店的
// 店长/大家长身份后，handleSubmit 仍然只认调用者自己 user_roles 里的真实
// 角色（可能不在 ALLOWED_SUBMIT_ROLES 白名单里，或者角色对但门店对不上），
// 服务端直接拒绝——这与前端 index.ts 的 refreshUserRoleView() 权限计算漏洞
// 是同一个根因在两端各自的表现（见 pages/index/lib/resolveActiveRoleGrant.js
// 头部注释），必须两端一起修，只改前端只是让按钮显示对了、提交仍然会失败。
// 本次补齐同一套 authorizedTenants 巡检漫游能力，不产生第二套不一致的鉴权
// 逻辑。
'use strict';
