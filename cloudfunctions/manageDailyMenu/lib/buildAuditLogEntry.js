...
// 需要连带改另一处。
//
// ⚠️（2026-09-11 第四处镜像）与 manageStoreProfile/lib/、
// getPatriarchDashboard/lib/、manageReportApproval/lib/ 下的同名文件是四处
// 独立维护的镜像（cloudfunctions/ 下每个云函数是独立部署单元，互相不能跨
// 目录 require），与本仓库 MERIT_TAGS/buildPurchasePlan 已有的"两处独立
// 维护同一份逻辑"是同一种既有约束的第四个实例。四份文件的函数行为必须
// 保持一致，改动一处记得同步其余三处。本次新增是为了让 manageDailyMenu
// 的采购清单/备餐预测能力也纳入 platform_admin 巡检漫游范围，与
// manageStoreProfile/getPatriarchDashboard/manageReportApproval 三个既有
// 消费点补齐同一套能力，不是从零发明新机制。
