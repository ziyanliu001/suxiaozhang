...
test('buildAuditLogEntry：channel 未传时兜底为 wechat_secret（本函数的默认通道）', () => {
  const entry = buildAuditLogEntry({ openid: 'o', success: true });
  assert.equal(entry.channel, 'wechat_secret');
});

test('buildAuditLogEntry：channel 显式传入时原样保留，供 cli_script/web_console 两条通道复用同一个构造器', () => {
  assert.equal(buildAuditLogEntry({ openid: 'o', success: true, channel: 'cli_script' }).channel, 'cli_script');
  assert.equal(buildAuditLogEntry({ openid: 'o', success: true, channel: 'web_console' }).channel, 'web_console');
});
