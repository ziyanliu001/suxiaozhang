...
//
// ⚠️（2026-09-14）本文件是 manageStoreProfile/lib/resolveCaller.js 的又一处
// 镜像（与 getPatriarchDashboard、manageReportApproval、manageDailyMenu、
// manageVolunteerSubmission 并列，见 docs/architecture/02_user_roles_single_document_invariant.md
// 第五节"不要假设方案三已覆盖全仓库，需要哪个就单独升级哪个"）。
//
// 🐛 根因（platform_admin 巡检时 SaaS 套餐门禁看不到目标机构真实状态）：
// checkTenantPermission 此前对 callerRole === 'platform_admin' 一律直接返回
// "企业版全放行"的豁免结果，完全不识别 authorizedTenants——platform_admin
// 巡检其他机构后，多门店看板/Excel 导出等付费墙形同虚设，看到的永远是
// 企业版豁免，而不是目标机构真实的 basic/pro/enterprise 门禁效果。本次引入
// 同一套 resolveEffectiveCaller 判定：命中有效巡检授权时，不再套用
// platform_admin 豁免，改用授权记录里的 tenantId/role 走真实套餐查询。
//
// index.js 里只需要 resolveEffectiveCaller(own, targetStoreId, now)——own 是
// 调用者自己那唯一一条 user_roles 文档（含 authorizedTenants 字段），
// targetStoreId 是本次请求携带的 event.storeId。
'use strict';
