...
// 明文写死、任何人（含开源后的公众读者）都能看到的默认值，而是直接拒绝执行签名
// 操作。此前"未配置则用默认值"的口径会让防篡改设计在忘记配置环境变量时形同虚设，
// 且一旦代码公开，默认值本身就是公开信息，相当于给所有没配置真实密钥的部署留了
// 一把公开的万能钥匙
const HMAC_SECRET = process.env.LEDGER_HMAC_SECRET || '';
if (!HMAC_SECRET) {
  console.error('[stampReportChecksum] 🚨 LEDGER_HMAC_SECRET 环境变量未配置，本云函数拒绝执行任何签名操作（fail-closed）。请立即在云开发控制台为本云函数配置真实密钥。');
}
const AMOUNT_TOLERANCE = 0.01;

function computeChecksum(item) {
  if (!HMAC_SECRET) {
    throw new Error('LEDGER_HMAC_SECRET 未配置，出于资金流水防篡改安全考虑，已拒绝执行签名操作，请联系管理员在云开发控制台配置该环境变量');
  }
