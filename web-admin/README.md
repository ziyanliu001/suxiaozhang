...
1. **启用"未登录用户访问云资源权限"**：微信云开发控制台 → 环境 → 权限设置，打开"未登录用户访问云资源权限"（未登录模式）。
   > ⚠️ 这与腾讯云独立版 CloudBase 的"匿名登录"身份源是**两回事**，不要混淆：本页面不调用、也不依赖 `auth.signInAnonymously()`——早期实现误以为需要先匿名登录换取登录态，结果在只开启了"未登录访问权限"（没有开启"匿名登录"身份源）的环境里被直接拒绝："请联系开发者在身份源列表开启匿名登录"。移除这一步之后又暴露出 `@cloudbase/js-sdk` (v2.28.6) 自身的一个缺陷：完全没有登录凭证时，`callFunction()` 内部会反复调用 `auth.getLoginState()`，未登录场景下该方法本身返回 `null`，SDK 没有对这个返回值做判空就直接读 `.scope`，在浏览器端直接抛异常崩溃、请求根本发不出去（真机含无痕模式复测均稳定复现，排除过本地缓存干扰）。现在的写法是给 `auth.getLoginState()` 打一个轻量兜底补丁——包一层原方法，解析出 `null` 或抛异常时统一兜底返回 `{ scope: '' }`，从根上中和这个判空缺失的 SDK bug，不产生、也不冒充任何登录态，真正的操作权限完全由 `adminWebAuth` 登录后签发的会话令牌决定。
...
- 已在真实浏览器（含无痕模式）里连续复现过三轮报错并针对性修复（`ERR_CERT_COMMON_NAME_INVALID` 证书报错 → 匿名登录被拒绝 → `callFunction` 内部 `null.scope` 崩溃，且第三轮的"预热 auth.getSession()"这个修复思路本身被证实无效，才改成现在这版直接给 `getLoginState()` 打补丁的方案），但**尚未有一轮"从头到尾完全跑通"的真机确认**——每一轮都是修复上一轮暴露的问题后交付，下一次真机测试才验证出下一层问题，如实标注这个"修复-复测-发现新问题"的迭代状态，不代表已经端到端验证过。
- **不再做地域自动探测**：早期版本曾经因为怀疑 `region` 参数猜错导致证书报错，加过一套自动依次尝试 `ap-shanghai`/`ap-guangzhou`/`ap-singapore` 三个地域的探测逻辑，且探测过程本身依赖 `auth.signInAnonymously()` 判断连通性。2026-09-13 确认该依赖本身就是错的（见上方部署前置条件的说明）后，探测逻辑随之整体移除——现在 `cloudbase.init()` 只在"连接设置"填了 `region` 时才会传这个参数，留空时使用 SDK 默认行为。至于 `ERR_CERT_COMMON_NAME_INVALID` 这类证书报错和后续两个报错之间是不是同一个根因的不同表现，还是排查过程中先后遇到的多个独立问题，目前没有足够证据下定论，如实标注这个不确定性。
- **`callFunction` 不再依赖"提前调用一次什么方法"去预热 SDK 状态，改为直接给 `auth.getLoginState()` 打补丁**：早期以为只需在 `callFunction` 前调用一次 `auth.getSession()`"预热"内部登录态即可规避 `null.scope` 崩溃，真机（含无痕模式）复测证实这个假设不成立——崩溃依然稳定复现，说明 `getLoginState()` 是被反复实时调用的，不是"预热一次就一劳永逸"的缓存值。现在的修复直接包一层 `auth.getLoginState()` 本身：原方法解析出 `null` 或抛异常时，统一兜底返回 `{ scope: '' }`，让内部拦截器永远读到一个非 `null` 的对象。这个具体的 SDK 内部行为（`callFunction` 内部反复调用 `getLoginState()`，未登录时返回 `null`）没有找到公开文档明确佐证，是根据连续几轮真机报错文本反推的判断，如实标注不确定性；这个补丁只影响"要不要崩溃"，不产生、也不冒充任何登录态。
- **常见排查线索**：若登录报"网络异常或环境配置有误"，先排除环境 ID 是否正确、静态托管是否已开启"未登录用户访问云资源权限"、部署域名是否在安全域名白名单内；若浏览器控制台明确看到 `ERR_CERT_COMMON_NAME_INVALID` 之类的证书报错，再尝试在"连接设置"里手动填一个 `region`；若看到 `Cannot read properties of null (reading 'scope')`，说明部署的不是包含 `getLoginState()` 补丁的最新版本，需要重新上传 `index.html` 并强刷缓存（建议用无痕窗口排除缓存干扰）。
